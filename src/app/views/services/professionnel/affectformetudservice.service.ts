...
export class AffectformetudserviceService {
  ...
  getAllForm(){
    ...
    return this.http.get<any>(`${this.URL}`+'formetude/getforms/',{headers: headers})  
  }
  getAffectation(idUser:any){
    const token=localStorage.getItem('token_Pro')
    const headers=new HttpHeaders().set('authorization','Bearer '+token)

    return this.http.get<any>(`${this.URL}`+'affectationFormEtudeToPro/getmyform/'+idUser,{headers: headers})  
